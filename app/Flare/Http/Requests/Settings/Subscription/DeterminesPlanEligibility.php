...
use FCLLA\Flare\Exceptions\IneligibleForPlan;
...
trait DeterminesPlanEligibility
{
    ...
    {
        $plan = Flare::plans()->where('id', $this->plan)->first();
        ...
        if (! $this->userIsEligibleForPlan($plan)) {
            $validator->errors()->add(
                'plan', trans('flare::validation.eligibility')
            );
        }
    }
    ...
    {
        try {
            if (! Flare::eligibleForPlan($this->user(), $plan)) {

...
$router->group(['middleware' => 'web'], function ($router) {
    ...
    // API Token Refresh...
    $router->put('/flare/token', 'TokenController@refresh');
    ...
    if (Flare::usesTeams()) {
        ...
        // Plans...
        $router->get('/flare/team-plans', 'TeamPlanController@all');
    ...
    // Plans...
    $router->get('/flare/plans', 'PlanController@all');
    ...
    /// Kiosk...
    $router->get('/flare/kiosk', 'Kiosk\DashboardController@show');

    // Kiosk Search...
    $router->post('/flare/kiosk/users/search', 'Kiosk\SearchController@performBasicSearch');

    // Kiosk Announcements...
    $router->get('/flare/kiosk/announcements', 'Kiosk\AnnouncementController@all');
    $router->post('/flare/kiosk/announcements', 'Kiosk\AnnouncementController@store');
    $router->put('/flare/kiosk/announcements/{id}', 'Kiosk\AnnouncementController@update');
    $router->delete('/flare/kiosk/announcements/{id}', 'Kiosk\AnnouncementController@destroy');

    // Kiosk Metrics / Performance Indicators...
    $router->get('/flare/kiosk/performance-indicators', 'Kiosk\PerformanceIndicatorsController@all');
    $router->get('/flare/kiosk/performance-indicators/revenue', 'Kiosk\PerformanceIndicatorsController@revenue');
    $router->get('/flare/kiosk/performance-indicators/plans', 'Kiosk\PerformanceIndicatorsController@subscribers');
    $router->get('/flare/kiosk/performance-indicators/trialing', 'Kiosk\PerformanceIndicatorsController@trialUsers');

    // Kiosk User Profiles...
    $router->get('/flare/kiosk/users/{id}/profile', 'Kiosk\ProfileController@show');

    // Kiosk Discounts...
    $router->post('/flare/kiosk/users/discount/{id}', 'Kiosk\DiscountController@store');

    // Kiosk Impersonation...
    $router->get('/flare/kiosk/users/impersonate/{id}', 'Kiosk\ImpersonationController@impersonate');
    $router->get('/flare/kiosk/users/stop-impersonating', 'Kiosk\ImpersonationController@stopImpersonating');

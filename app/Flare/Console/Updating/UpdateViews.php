...
class UpdateViews
{
    ...
    /**
     * Get all of the current views for the Flare installation.
     *
    ...
    {
        $views = collect(
            (new Filesystem)->allFiles(FLARE_PATH.'/resources/views')
        );
    ...
    /**
     * Publish the Flare views that exist in the download but are not published.
     *
     * @return void
     */
    ...
    {
        return resource_path('views/vendor/flare/'.$this->relativeViewPath($view));
    }
    ...
    {
        $viewPath = str_replace(FLARE_PATH.'/resources/views/', '', $view->getRealPath());

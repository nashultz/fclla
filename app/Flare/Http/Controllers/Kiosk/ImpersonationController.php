...
class ImpersonationController extends Controller
{
    ...
    public function impersonate(Request $request, $userId)
    {
        ...
        $request->session()->put(
            'flare:impersonator', $request->user()->id
        );

        Auth::login(Flare::user()->findOrFail($userId));
    ...
    public function stopImpersonating(Request $request)
    {
        ...
        if (! $request->session()->has('flare:impersonator')) {
            Auth::logout();
        ...
        return redirect('/flare/kiosk#/users/'.$currentId);
    }
}
